...
public class TangibleAssetFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_tangible_asset, container, false);
                ...
                btnRegisterAsset.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (TextUtils.isEmpty(edtFixedAssetName.getText().toString())) {
                            ...
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_name").setValue(edtFixedAssetName.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_type").setValue(btnAssetType.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_purchased_day").setValue(edtBthDay.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_purchased_month").setValue(edtBthMonth.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_purchased_year").setValue(edtBthYear.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_price").setValue(edtPrice.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_quantity").setValue(edtQuantity.getText().toString());
                            companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("timestamp").setValue(ServerValue.TIMESTAMP);

                            if (btnAssetType.getText().toString().equals("Ganado de trabajo y reproducción; redes de pesca")) {
                                companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_depreciation_rate").setValue("25");
                            }
                            if (btnAssetType.getText().toString().equals("Vehículos de transporte terrestre (excepto ferrocarriles); hornos en general")) {
                                companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_depreciation_rate").setValue("20");
                            }
                            if (btnAssetType.getText().toString().equals("Maquinaria y equipo utilizados por las actividades minera, petrolera y de construcción, excepto muebles, enseres y equipos de oficina")) {
                                companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_depreciation_rate").setValue("20");
                            }
                            if (btnAssetType.getText().toString().equals("Equipos de procesamiento de datos")) {
                                companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_depreciation_rate").setValue("25");
                            }
                            if (btnAssetType.getText().toString().equals("Maquinaria y equipo adquirido a partir del 1.1.1991")) {
                                companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_depreciation_rate").setValue("10");
                            }
                            if (btnAssetType.getText().toString().equals("Otros bienes del activo fijo")) {
                                companyRef.child(post_key).child("Fixed Assets").child("Tangible").child(timestamp).child("asset_depreciation_rate").setValue("10");
                            }
    ...
    private void showFixedAssets() {
        Query query = companyRef.child(post_key).child("Fixed Assets").child("Tangible").orderByChild("timestamp");
            ...
            protected void populateViewHolder(final FixedAssetTangibleViewHolder viewHolder, FixedAssetTangibleModel model, int position) {
                ...
                viewHolder.txtShowDeprecation.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        LayoutInflater inflater = LayoutInflater.from(getActivity());
                        View finance_method = inflater.inflate(R.layout.depreciation_value_dialog,null);

                        TextView txtDate,txtAssetName,txtDeprecationRate,txtNetValue,txtDeprecationPeriod;

                        txtDate = finance_method.findViewById(R.id.txtDate);
                        txtAssetName = finance_method.findViewById(R.id.txtAssetName);
                        txtDeprecationRate = finance_method.findViewById(R.id.txtDeprecationRate);
                        txtNetValue = finance_method.findViewById(R.id.txtNetValue);
                        txtDeprecationPeriod = finance_method.findViewById(R.id.txtDeprecationPeriod);

                        txtDate.setText("VALOR NETO AL "+day+"/"+month+"/"+year);
                        txtAssetName.setText("Activo: "+viewHolder.my_asset_name);
                        txtDeprecationRate.setText("Tasa Anual de Depreciación: "+viewHolder.my_asset_depreciation_rate+"%");

                        double annual_deprecation = deprecation_rate*total;
                        double monthly_deprecation = annual_deprecation/12;
                        String start = viewHolder.my_asset_purchased_day+"/"+viewHolder.my_asset_purchased_month+"/"+viewHolder.my_asset_purchased_year;
                        ...
                        try {
                            ...
                            txtDeprecationPeriod.setText("Depreciación Acumulada: S/ "+net_value_st);

                            double real_net_value = total-net_value;
                            String real_net_value_st = decimalFormat.format(real_net_value);
                            txtNetValue.setText("S/ "+real_net_value_st);
